...
function _openGroupSheet(g, onReload) {
  ...
  sheet.innerHTML = /* html */`
    ...
    <div class="life-sheet-panel" role="dialog" aria-label="${isNew ? 'New Group' : 'Edit Group'}">
      ...
      <div class="life-sheet-foot">
        ${!isNew ? `
          <div style="display:flex;gap:6px;margin-right:auto;align-items:center">
            <button class="flock-btn flock-btn--danger" data-archive title="Archive this group">
              <svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 8v13H3V8"/><path d="M1 3h22v5H1z"/><path d="M10 12h4"/></svg>
              Archive
            </button>
            <button class="flock-btn" data-hard-delete title="Permanently delete this group"
              style="padding:6px 8px;color:#dc2626;border-color:#fca5a5">
  ...
  sheet.querySelector('[data-archive]')?.addEventListener('click', async () => {
    const ok = confirm(`Archive "${name}"?\n\nThe group will be hidden from active lists but not permanently deleted.`);
    if (!ok) return;
    ...
    try {
      ...
      btn.disabled = false;
      btn.innerHTML = `<svg width="13" height="13" viewBox="0 0 24 24" fill="none" stroke="currentColor" stroke-width="2.2" stroke-linecap="round" stroke-linejoin="round"><path d="M21 8v13H3V8"/><path d="M1 3h22v5H1z"/><path d="M10 12h4"/></svg> Archive`;
    }
  });

  sheet.querySelector('[data-hard-delete]')?.addEventListener('click', async () => {
    const first = confirm(`Permanently delete "${name}"?\n\nThis cannot be undone — all group data will be removed from the database.`);
    if (!first) return;
    const second = confirm(`Are you sure? "${name}" will be permanently deleted.`);
    if (!second) return;
    const btn = sheet.querySelector('[data-hard-delete]');
    btn.disabled = true;
    try {
      await MXG.delete({ id: uid });
      _closeWaySheet();
      onReload?.();
    } catch (err) {
      console.error('[TheWay] group delete error:', err);
