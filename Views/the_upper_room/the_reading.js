...
async function _fetch() {
  // 1. Static bundle (generated by generate_one_year_bible.py).
  try {
    const mod = await import('../Data/one_year_bible.js');
    const rows = mod.default || [];
    if (Array.isArray(rows) && rows.length) return _normalise(rows);
...
function _normalise(rows) {
  return rows.map((r, i) => ({
    ...
    psalms:     r.ps || r.psalms || r.Psalms || '',
    proverbs:   r.pr || r.proverbs || r.Proverbs || '',
